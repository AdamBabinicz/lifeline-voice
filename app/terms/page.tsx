    ...
    <main className="min-h-screen bg-background px-6 py-12 pb-32 text-foreground sm:px-10 lg:px-16">
      <div className="mx-auto flex max-w-3xl flex-col gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <h1 className="text-4xl font-black uppercase tracking-tight sm:text-6xl">{t.terms_title}</h1>
          <p className="max-w-2xl text-lg leading-relaxed text-muted-foreground">{t.terms_intro}</p>
        </div>
        <section className="flex flex-col gap-4 border border-border bg-card p-6 leading-relaxed text-muted-foreground sm:p-8">
          <p>{t.terms_body_one}</p>
          <p>{t.terms_body_two}</p>
        </section>
      </div>
    </main>
  )
